...
insert into location values("loc4","location4","bayridge","100");
insert into location values("loc5","location5","long island","200");
insert into location values("loc6","location6","coney island","300");
...
insert into concert values("concert1","band1","g1","loc1",0,10,20141124080000,20141126080000);
insert into concert values("concert2","band2","g2","loc2",0,20,20141125080000,20141127080000);
insert into concert values("concert3","band3","g3","loc3",0,30,20141126080000,20141128080000);
...
delete from concert where cid="concert4";
select * from concert;
...
select X.userid,count(*) from ( select u.userid , DATEDIFF(r.rtime,u.starttime) as trust,count(*) as ratecount
from user u natural join user_rating r group by u.userid) X where X.trust > 100 and X.ratecount>2 
group by X.userid;
DELIMITER $$
CREATE PROCEDURE insert_user_concert(In cid varchar(20) ,In bid varchar(20),In gid varchar(20), 
In venue varchar(20), in tktsold int, in tprice int, in ctime datetime,in publishtime datetime)
BEGIN
declare count int default 0;
select count(*) into count from ( select u.userid , DATEDIFF(r.rtime,u.starttime) as trust,count(*) as ratecount
from user u natural join user_rating r group by u.userid) X where X.trust > 100 and X.ratecount>2 
group by X.userid;
if count>0 then
Set publishtime=now();
...
END $$
CALL insertBand("concert5","band3","g1","loc1",2,30,20141126080000,20141128080000);
select * from concert;
...
select * from rec_concert;
drop procedure if exists concertrecommendGenre;

DELIMITER $$
create procedure concertrecommendGenre(in usernumber varchar(20))
begin
select cid as recommendedConcert from concert natural join user where gid not in
(
select gid from user natural join genre_user 
where userid=usernumber
) and userid=usernumber ;
END $$
DELIMITER ;

call concertrecommendGenre("user1");
select * from concert;

drop procedure if exists concertrecommendBand;

DELIMITER $$
create procedure concertrecommendBand(in usernumber varchar(20))
begin
select cid from concert, (
select bid from user_band where userid !=usernumber)
X where X.bid =concert.bid ;
END $$
DELIMITER ;
call concertrecommendBand("user1");
